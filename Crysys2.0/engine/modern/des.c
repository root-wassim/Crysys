...
static void permute(const unsigned char* in, unsigned char* out, const int* table, int tableLen) {
...
static void generateKeys(const char* key, unsigned char keys[16][48]) {
    ...
    for (int round = 0; round < 16; round++) {
        ...
        permute(combined, keys[round], pc2, 48);
    }
}
...
static void des_process_block(const unsigned char* block, unsigned char keys[16][48], int decrypt, unsigned char* result) {
    ...
    for (int i = 0; i < 64; i++) {
        blockBits[i] = (block[i / 8] >> (7 - (i % 8))) & 1;
    }
    
    unsigned char permuted[64];
    permute(blockBits, permuted, initialPermutation, 64);
    ...
    for (int round = 0; round < 16; round++) {
        ...
        permute(right, expanded, expansionTable, 48);
        
        unsigned char xored[48];
        for (int i = 0; i < 48; i++) {
            xored[i] = expanded[i] ^ keys[keyIndex][i];
        }
        ...
        permute(substituted, pBoxResult, pBox, 32);
        
        unsigned char temp[32];
        memcpy(temp, right, 32);
        for (int i = 0; i < 32; i++) {
            right[i] = left[i] ^ pBoxResult[i];
        }
        memcpy(left, temp, 32);
    }
    ...
    unsigned char finalBits[64];
    permute(combined, finalBits, finalPermutation, 64);
