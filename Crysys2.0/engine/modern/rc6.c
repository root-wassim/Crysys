...
static uint32_t rotl(uint32_t val, uint32_t shift) {
    shift = shift % 32;
...
static void init_rc6(RC6Algo* rc6, const char* key) {
    ...
    for (int k = 0; k < v; k++) {
        A = rc6->S[i_idx] = rotl(rc6->S[i_idx] + A + B, 3);
        B = L[j_idx] = rotl(L[j_idx] + A + B, A + B);
        i_idx = (i_idx + 1) % (2 * (rc6->r + 2));
        j_idx = (j_idx + 1) % Lsize;
    }
    free(L);
}
...
static void rc6_encrypt_block(RC6Algo* rc6, const unsigned char* block, unsigned char* result) {
    ...
    for (int i = 1; i <= rc6->r; i++) {
        ...
        A = rotl(A ^ t, u) + rc6->S[2 * i];
        C = rotl(C ^ u, t) + rc6->S[2 * i + 1];
...
static void rc6_decrypt_block(RC6Algo* rc6, const unsigned char* block, unsigned char* result) {
    ...
    for (int i = rc6->r; i >= 1; i--) {
        ...
        C = rotr(C - rc6->S[2 * i + 1], t) ^ u;
        A = rotr(A - rc6->S[2 * i], u) ^ t;
    }
